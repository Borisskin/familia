...
@pytest.mark.asyncio
async def test_runtime_pair_write_bypasses_legacy_policy_after_graph_gate(
    monkeypatch: pytest.MonkeyPatch,
    registry: PrincipalRegistry,
) -> None:
    requests: list[dict[str, Any]] = []
    ...
    result = await MemorySetTool().execute(
        scope="pair:actor_alpha",
        key="value:memory",
        value="pair fact",
    )
...
@pytest.mark.asyncio
async def test_dream_pair_rejects_principals_without_peer_relation(
    monkeypatch: pytest.MonkeyPatch,
    registry: PrincipalRegistry,
) -> None:
    requests: list[dict[str, Any]] = []
    monkeypatch.setattr(dream_memory_mod, "is_peer", lambda _left, _right: False, raising=False)
    monkeypatch.setattr(
        dream_memory_mod,
        "get_engine",
        lambda: SimpleNamespace(
    ...
    result = await dream_memory_mod.DreamMemorySetTool(api_key="dream-key").execute(
        scope="pair",
        actor="actor_alpha",
        other="actor_zeta",
        key="value:memory",
    ...
    assert result == "Skipped: pair Dream item requires an allowed peer relationship"
    assert requests == []


@pytest.mark.asyncio
async def test_dream_pair_bypasses_legacy_policy_after_graph_gate(
    monkeypatch: pytest.MonkeyPatch,
    registry: PrincipalRegistry,
) -> None:
    requests: list[dict[str, Any]] = []
    monkeypatch.setattr(dream_memory_mod, "is_peer", lambda _left, _right: True, raising=False)
    monkeypatch.setattr(
        dream_memory_mod,
        "get_engine",
        _legacy_policy_must_not_run,
...
@pytest.mark.asyncio
async def test_dream_local_skips_do_not_block_later_exact_actor(
    monkeypatch: pytest.MonkeyPatch,
    registry: PrincipalRegistry,
) -> None:
    requests: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: _RecordingClient(
            requests,
            get_response=_Response(200, None),
            post_response=_Response(
                200,
                {
                    "ok": True,
    ...
    items = [
        {"scope": "private", "actor": None, "key": "note", "value": "actorless"},
        {"scope": "private", "actor": "actor_unknown", "key": "note", "value": "unknown"},
        {"scope": "malformed", "actor": "actor_alpha", "key": "note", "value": "bad"},
        {"scope": "private", "actor": "actor_alpha", "key": "note", "value": "   "},
        {
            "scope": "private",
            "actor": "actor_zeta",
            "key": "value:memory",
            "value": "valid actor fact",
        },
    ]
    ...
    assert all(result.startswith("Skipped:") for result in results[:-1])
    assert results[-1] == "Stored at 'private:actor_zeta:value:memory'"
    assert requests == [
        {
            "key": "private:actor_zeta:value:memory",
            "value": "valid actor fact",
            "expected_ts": None,
        }
    ]
...
@pytest.mark.asyncio
async def test_dream_systemic_memx_failure_remains_fatal_and_observable(
    monkeypatch: pytest.MonkeyPatch,
    registry: PrincipalRegistry,
) -> None:
    requests: list[dict[str, Any]] = []
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: _RecordingClient(
            requests,
            get_response=_Response(503, {"detail": "down"}),
            post_response=_Response(500, {"detail": "unexpected POST"}),
        ),
    )
    set_current_actor(dream_memory_mod.CONSOLIDATOR_ACTOR)
    tool = dream_memory_mod.DreamMemorySetTool(api_key="dream-key")
    ...
    assert result.startswith("Error: memX 503")
    assert requests == []
