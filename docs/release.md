...
Каждый выпуск админки объявляет минимальную версию образа (константа `min_image_version`, сейчас `0.4.1`). `/health` у gateway возвращает `image_version`. При подключении админка сравнивает версии; если есть расхождение, баннер **Compatibility** предлагает встроенную кнопку обновления образа.
