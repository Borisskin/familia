...
`bootstrap.sh MODE=update` выполняет переход в строгом порядке:

1. останавливает gateway и остальные процессы записи;
2. собирает новый образ;
3. запускает только memX, согласует список доступа, перезапускает memX и ждёт
   его готовности;
4. один раз выполняет переход памяти:

```bash
familia migrate hybrid-storage --apply --json
```

5. только при состоянии `complete` запускает gateway, ждёт его готовности,
   исправляет права и очищает переходные данные.

Состояния `partial` и `failed` прерывают обновление: gateway не запускается, а
версия не меняется. `prereqs`, `docker` и `probe_mirrors` сохраняются на
случай изменений ВМ или требований к зеркалам.

### Атомарный `SOURCE_VERSION`

`/opt/familia/SOURCE_VERSION` содержит SemVer бэкенда и пишется только после
полного успеха всей последовательности: переход памяти завершён, gateway
запущен и здоров, права исправлены, переходные данные очищены. Если любой шаг
не завершён, файл остаётся со старым значением.
...
- [`policy.md`](policy.md) — правила частной записи и чтения по темам.
