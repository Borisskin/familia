...
class TestSandboxPlatform:
    ...
    async def test_private_scope_rejects_unavailable_or_empty_sandbox(
        self, tmp_path, is_windows, sandbox
    ):
        ...
        try:
            with (
                patch("nanobot.agent.tools.shell._IS_WINDOWS", is_windows),
                patch.object(ExecTool, "_spawn", new_callable=AsyncMock) as mock_spawn,
            ):
                result = await ExecTool(
                    sandbox=sandbox,
                    working_dir=str(tmp_path),
                ).execute(command="echo private")
        finally:
            reset_workspace_scope(token)

        assert "private workspace exec" in result
        mock_spawn.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_private_scope_keeps_bwrap_on_linux(self, tmp_path):
        scope = build_workspace_scope(
            tmp_path,
            "restricted",
            allow_shared_extras=False,
        ...
        token = bind_workspace_scope(scope)
        mock_proc = AsyncMock()
        mock_proc.communicate.return_value = (b"private-sandboxed", b"")
        mock_proc.returncode = 0
        try:
            with (
                patch("nanobot.agent.tools.shell._IS_WINDOWS", False),
                patch(
                    ...
                    return_value="bwrap -- sh -c echo private",
                ) as mock_wrap,
        ...
        assert "private-sandboxed" in result
