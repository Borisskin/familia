...
@pytest.mark.asyncio
async def test_trigger_now_uses_injected_source_reader(tmp_path) -> None:
    (tmp_path / "HEARTBEAT.md").write_text("legacy file task", encoding="utf-8")
    provider = DummyProvider([
        LLMResponse(
            content="",
            tool_calls=[
                ToolCallRequest(
                    id="hb_1",
                    name="heartbeat",
                    arguments={"action": "run", "tasks": "memx task"},
                )
            ],
        )
    ])
    ...
    def read_source() -> tuple[str | None, str | None]:
        return "memx task", "memx"
...
def test_injected_source_reader_does_not_fall_back_to_file(tmp_path) -> None:
    ...
    service = HeartbeatService(
        ...
        provider=DummyProvider([]),
        model="test-model",
        source_reader=lambda: (None, None),
    )
...
def test_injected_source_reader_returns_content(tmp_path) -> None:
    ...
    service = HeartbeatService(
        ...
        provider=DummyProvider([]),
        model="test-model",
        source_reader=lambda: ("memx content", "memx"),
    )
...
def test_file_fallback_used_without_source_reader(tmp_path) -> None:
    (tmp_path / "HEARTBEAT.md").write_text("file content", encoding="utf-8")
