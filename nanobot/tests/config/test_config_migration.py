...
@pytest.mark.parametrize("field_name", ["memoryWindow", "memory_window"])
def test_load_config_keeps_max_tokens_and_ignores_legacy_memory_window(
    tmp_path, field_name
) -> None:
...
@pytest.mark.parametrize("field_name", ["memoryWindow", "memory_window"])
def test_save_config_writes_context_window_tokens_but_not_memory_window(
    tmp_path, field_name
) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        json.dumps(
            {
                "agents": {
                    ...
                    "familiaFallback": {"model": "family/test"},
                },
            }
        ),
        encoding="utf-8",
    ...
    assert saved["agents"]["familiaFallback"] == {"model": "family/test"}
