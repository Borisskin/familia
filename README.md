...
> обновить одной кнопкой. Новая версия фиксируется только после полного
> завершения перехода памяти; резервное копирование и восстановление описаны
...
- **Кто их видит:** только вы и те, кому вы дали SSH-доступ к VM. Бэкенд НЕ
  ...
  мессенджеров.
- **Как делится память между людьми:** новая запись всегда принадлежит
  говорящему. В автоматический контекст другого человека попадает только имя
  конкретного `memory:<fact_id>`, когда совпали точная запись личного каталога
  владельца, прямая поддерживаемая семейная связь и подтверждённый общий топик;
  значение читается точечно с повторной проверкой тех же условий. Профиль,
  служебная и секретная память остаются доступны только владельцу. Темы
  создаются и связываются только в админке. Подробнее — в
  [правилах доступа](docs/policy.md).
...
Каждый человек видит свою память. Родственная связь, старые общие или парные
области, статическое правило и отсутствие метки сами по себе не открывают
чужую запись. Чужое атомарное имя доступно только после совпадения каталога,
прямой связи и общего топика; роль администратора в чате не разрешает запись
в чужую память.
...
[выпуска](https://github.com/Verconto/familia/releases/latest). Рядом обязательно положи `WebView2Loader.dll`
из того же релиза — иначе админка не запустится.

Пока есть только Windows-сборка. Mac и Linux — позже. Можно выбрать переносимую
программу или установщик. SmartScreen ругнётся при первом запуске —
